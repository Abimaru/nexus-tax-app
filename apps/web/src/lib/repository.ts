...
export async function deleteDocumentPermanently(documentId: string): Promise<void> {
  ...
  // Fase 1: borrado en cascada del documento y sus dependencias exclusivas.
  // Se hace en una transacción con los mismos tables para atomicidad; Dexie 4
  // acepta hasta 7 argumentos posicionales, así que agrupamos aquí y hacemos
  // las tablas restantes (facts, employmentGroups) en transacciones aparte.
  await db.transaction(
    'rw',
    db.documents,
    db.documentBlobs,
  ...
  // Fase 2: los hechos son decisiones humanas; se conservan, pero pierden su
  ...
  // Fase 3: retirar la referencia del documento de las instancias de empleador.
  await db.transaction('rw', db.employmentGroups, async () => {
